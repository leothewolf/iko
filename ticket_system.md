...
* -tstart
> tstart command is used to start Ticket System for that server<br><br>How to use -tstart command?<br>Choose a channel in your server from where tickets will be created and from where the ticket system can be managed, let the name of channel be 'create-tickets' (can be any). Next for setting up Ticket System<br><br> **Note : You need to mention the role id and not the role itself!**<br><br>If you want to setup Ticket System which has embed without image just type:<br><br>```-tstart -r support_role_id```<br><br>If you want to setup Ticket System which has embed with image just type:<br><br>```-tstart -r support_role_id -i image_link```
...
* -trole
> trole command is used to change the currently assigned support role<br><br>To change the role run the following command<br><br>```-trole -r support_role_id```<br><br>**Note : You don't need to tag the role, you just need to type the id of the role**
